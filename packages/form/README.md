...
- **Current Implementation (P1e — Validation, Parsers & Standard Schema):**
  - **Leaf Field Primitive (`createField`):**
    ...
    - Reactive state signals: `value`, `rawValue`, `initialValue`, `initialRawValue`, `touched`, `dirty`, `pending`, `valid`, `invalid`, `issues`, `parseIssue`, `parseStatus`, `validationStatus`.
  - **Built-in Parsers:**
    ...
    - `createStringParser`, `createOptionalStringParser`, `createBooleanParser`.
    - `sanitizeParseIssue` and `sanitizeValidationIssue` with prototype pollution defense.
  - **Nested Groups (`createFieldGroup`) & Root Forms (`createForm`):**
    ...
    - Whole-form baseline reinitialization (`form.reinitialize(newBaseline)`) and batched reset.
