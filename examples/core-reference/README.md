...
This is the first real reference consumer for the experimental `@vii/core` release path. It models
a tiny checkout quantity flow using State, Computed, and Scope-owned subscriptions.

Run it from the repository with:

```bash
pnpm --filter @vii/core-reference test
pnpm --filter @vii/core-reference build
```
