...
Slice P1j implements the production Angular 17+ and Vue 3.3+ reactive adapters (`@vii-labs/form/angular` and `@vii-labs/form/vue`). Features include: zero-state-mirror reactive handles (`createAngularField`, `toAngularField`, `createAngularForm`, `createAngularFieldArray` for Angular; `createVueField`, `useViiField`, `createVueForm`, `createVueFieldArray` for Vue); idiomatic framework primitives (Angular `Signal<T>` with fine-grained equality, Vue `Readonly<ShallowRef<T>>`); stable action references and aggregate state synchronization; caller-owned error containment; explicit lifecycle disposal via manual `.dispose()` and automatic framework lifecycle integration (Angular `DestroyRef` / `inject(DestroyRef)` throwing deterministically outside injection context, Vue `onScopeDispose` when inside `effectScope` with `useViiField` requiring an active scope); preservation of canonical Form node lifecycles without auto-disposal on framework component teardown; and clean package subpath separation keeping root `@vii-labs/form` framework-agnostic.
