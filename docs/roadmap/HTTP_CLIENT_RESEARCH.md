...
| --- | --- | --- |
| **H0** | **Architecture + Semantic Boundaries** | Establish transport contracts, Fetch-first baseline, client ownership, error taxonomy, cancellation/timeout ownership, middleware model, Query/Schema boundaries, SSR isolation, security baseline, and build-vs-buy criteria. *(Completed in PR #128)* |
| **H1** | **Fetch-first Client Baseline** | Prototype the smallest explicit client (`createHttpClient`), injected Fetch capability, deterministic URL/header composition, request/response lifecycle, and raw `Response` access. *(Completed in PR #129)* |
| **H2** | **Middleware / Request Pipeline** | Research functional middleware (`(request, next) => Promise<Response>`), deterministic onion-style execution order, short-circuiting, context propagation, and failure isolation. *(Completed in PR #130)* |
| **H3** | **Cancellation + Timeout + Scope** | Research unified cancellation via native `AbortSignal`, signal composition (`composeSignals`), `createTimeoutSignal`, Vii `Scope` lifecycle binding, and resource cleanup. *(Completed in PR #131)* |
| **H4** | **Errors + Body Decode + Standard Schema Boundary** | Prototype normalized error classification (`HttpError`, `NetworkError`, `TimeoutError`, `AbortError`, `HttpParseError`, `HttpValidationError`), explicit body decoders, Standard Schema v1 validation, and privacy-safe diagnostics. *(Completed in PR #132)* |
| **H5** | **Retry + Idempotency Research** | Research retry-safety contracts (disabled by default), HTTP method idempotency, `Retry-After` headers, exponential backoff with jitter, non-replayable body constraints, and Query ownership separation. *(Completed in PR #133)* |
| **H6** | **Streaming / Upload / Backpressure** | Research native Web Streams (`ReadableStream`), WHATWG SSE framing parser (`parseEventStream`), line iteration across chunk boundaries, and cancellation reader cleanup. *(Completed in PR #134)* |
| **H7** | **SSR Isolation + Security Hardening** | Research request-scoped client factories in SSR runtimes, SSRF preflight policy (`validateUrlSecurity`), private IP/cloud metadata restrictions, and cross-origin header sanitization. *(Completed in PR #137)* |
| **H8** | **Observability + Tracing + Metrics** | Research lifecycle hooks (`onRequest`, `onResponse`, `onError`), W3C Trace Context propagation (`traceparent`), structured duration timing (`durationMs`), and sensitive query/header logging redaction. *(Completed in PR #138)* |
| **H8R** | **Runtime Compatibility Evidence** | Execute portable compatibility contract across Chromium (Chrome 133 via CDP), Node.js (v22.17.0), and Bun (v1.2.18); document explicit blockers for Firefox, WebKit, Deno, and Worker environments. *(Documented in `docs/quality/HTTP_RUNTIME_COMPATIBILITY.md`)* |
| **H9R** | **Build-vs-Buy Benchmark Evidence** | Execute reproducible benchmarks against Native Fetch, handwritten helper, `ky@1.7.5`, `ofetch@1.4.1`, and `axios@1.18.0`; measure bundle footprint, client creation, JSON decode throughput, and verify competitor retry defaults. *(Documented in `docs/quality/HTTP_BUILD_VS_BUY_EVIDENCE.md`)* |
| **H9** | **Graduation Gate + Build-vs-Buy Decision** | Author final architecture decision record and render formal graduation verdict: `Wrap + Reduce` (Zero-Dependency Micro-Transport Primitive). *(Documented in `docs/architecture/ADR_HTTP_GRADUATION_DECISION.md`)* |
