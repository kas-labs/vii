...
status, and resource disposal success; names, values, other payload fields, files, network, and
mutation authority are outside its output interface. A bounded trace may contain a disposed node
without its creation event; the inspector preserves that partial structural evidence. It does not
stabilize the trace schema or introduce a terminal CLI command.
