...
Slice P2f implements Angular ecosystem integrations for `@vii-labs/form` (`@vii-labs/form/angular`). Signal Forms remain formally deferred (Option A). Features include: `ViiFieldDirective` (`[viiField]`) authored with public `@Directive` / `@Input` APIs and partial `ngc` compilation (host `ElementRef` injected by Angular; no private Ivy authoring in source); real TestBed template acceptance for input, textarea, and checkbox; `ViiControlValueAccessor` as a genuine Angular Forms `ControlValueAccessor` wired via `NG_VALUE_ACCESSOR` with FormControl anti-loop tests; scoped DI via `VII_FORM_TOKEN`, `provideViiForm`, and `injectViiForm`; presentation-only teardown invariant; 100-field isolation and 1,000-cycle stress verification; 41/41 hard performance gates (Angular adapter within gzip/brotli budgets); packed consumers for Angular 17.3.12 and modern Angular compile and exercise real `[viiField]` templates from the published tarball.
