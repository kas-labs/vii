...
Slice P1i implements the production Vanilla DOM adapter (`@vii-labs/form/vanilla`), exposing `bindField` and `bindForm`. Features include: single commit event contract (`input` for text-like controls, `change` for checkbox/radio/select/file without duplicate event registration); bidirectional synchronization without feedback loops; `rawValue` presentation retention preventing input snap-backs on parsed fields; `aria-invalid` projection strictly when invalid (client, parse, server) and never on pending-only; additive and restorative `aria-describedby` token linking with issue element IDs; safe plain text sinks (`textContent`) guaranteeing DOM XSS defense; submit error ownership and unexpected rejection containment (`onSubmitException`) preventing unhandled Promise rejections; transactional preflight throwing `TypeError` before mutation; and deterministic lifecycle disposal removing listeners and subscriptions without destroying canonical Form nodes.
