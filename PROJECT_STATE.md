...
Slice P2f implements Angular ecosystem integrations for `@vii-labs/form` (`@vii-labs/form/angular`). Preserving strict Angular 17.3.12 compatibility without introducing experimental compiler dependencies or runtime bloat, Signal Forms integration was formally deferred (Option A). Features include: `ViiFieldDirective` (`[viiField]`) standalone Ivy directive (`ɵdir`) providing two-way DOM binding for `<input>` and `<textarea>`, event synchronization, blur/touch handling, runtime type narrowing (`isBooleanField`, `isStringField`), fail-closed safety, SSR compatibility, and zero-leak listener teardown; `ViiControlValueAccessor` / `createViiControlValueAccessor` CVA bridge with reentrancy guard (`isPropagatingFromAngular`) preventing infinite ping-pong loops, presentation-owned `disabled` signal, and isolated lifecycle cleanup; scoped DI via `VII_FORM_TOKEN`, `provideViiForm`, and `injectViiForm`; strict preservation of the core teardown invariant (presentation destruction never disposes canonical `FieldState` or `FormInstance`); 100-field isolation and 1,000-cycle stress verification with 0 retained subscriptions; full pass across all 41/41 hard performance, bundle, and memory gates with zero budget weakening (Angular adapter: 6,312 B minified, 1,883 B gzip, 1,682 B brotli); and clean consumer verification across all 8 clean packed workspace targets (including Angular 17.3.12 and modern Angular).
