...
package-boundary tests, tarball validation, and clean consumer validation. Slice P1c implemented the
...
Slice P1g implements the Model A form submission lifecycle and structured server issue routing. Features include: Model A submission state machine (`idle -> validating -> submitting -> succeeded | failed | cancelled`) where terminal outcome statuses persist across subsequent field edits without resetting to idle; validation gate blocking submit when invalid or when any field has parser errors; immutable domain output snapshotting (`deepCloneSnapshot`) defending against in-flight tree mutations; caller error ownership re-throwing unexpected submit action rejections while marking status as failed; AbortSignal cancellation (`form.cancelSubmit()`, duplicate policies `supersede`/`drop`/`reject`, automatic abort on reset/reinitialize/dispose); structured `ServerIssue` taxonomy with routing to leaf fields, nested groups, arrays, and fallback to root `form.serverIssues`; localized server issue clearing on field edit (`setValue`/`setRawValue`) without disturbing sibling or root issues; `FieldArray` submission-time identity snapshots (`collectArraySnapshots`) routing server responses to stable item IDs at their live position across mid-flight reorders and falling back deleted items safely to root `form.serverIssues`; and value-free structural diagnostics privacy.
Slice P1h implements the production React 18/19 adapter (`@vii-labs/form/react`), exposing `useField`, `useForm`, and `useFieldArray` via `useSyncExternalStore`. Features include: live snapshot reads with referential property memoization preventing re-render loops; pre-subscription freshness; fine-grained render isolation (0 sibling re-renders on independent leaf edits); stable item ID preservation (`FieldArrayItem.id`) across collection reordering; StrictMode double-mount lifecycle safety; clean unmount unsubscription without auto-disposing canonical Form nodes; node replacement (`fieldA` -> `fieldB`) without cross-talk; deterministic SSR execution (`getServerSnapshot`); and strict import isolation preserving framework-neutral root `@vii-labs/form`.
Slice P1i implements the production Vanilla DOM adapter (`@vii-labs/form/vanilla`), exposing `bindField` and `bindForm`. Features include: supported form control classification (input, textarea, select-one) with transactional fail-closed `TypeError` preflight on unsupported elements (div, span, button, select-multiple, arbitrary objects); single commit event contract (`input` for text-like controls, `change` for checkbox/radio/select-one/file without duplicate event registration); bidirectional synchronization without feedback loops; `rawValue` presentation retention preventing input snap-backs on parsed fields; non-destructive `aria-invalid` projection capturing pre-binding attribute state and restoring the original application value (or removing it if absent) upon field validity or disposal; additive and restorative `aria-describedby` token linking with issue element IDs; safe plain text sinks preventing HTML interpretation at the issue-message sink by writing through `textContent`; submit error ownership and unexpected rejection containment (`onSubmitException`) preventing unhandled Promise rejections; and deterministic lifecycle disposal removing listeners and subscriptions without destroying canonical Form nodes. Multi-select (`select-multiple`) is explicitly deferred from P1i and fails closed.
Slice P1j implements the production Angular 17+ and Vue 3.3+ reactive adapters (`@vii-labs/form/angular` and `@vii-labs/form/vue`). Features include: no duplicate domain-state ownership (`createAngularField`, `createAngularForm`, `createAngularFieldArray` for Angular; `createVueField`, `createVueForm`, `createVueFieldArray` for Vue); idiomatic framework primitives (Angular `Signal<T>` with fine-grained equality, Vue `Readonly<ShallowRef<T>>`); exact 12 read-only framework signals/refs per field (`value`, `rawValue`, `dirty`, `touched`, `pending`, `valid`, `invalid`, `parseStatus`, `parseIssue`, `validationStatus`, `issues`, `serverIssues`) without a redundant aggregate state signal; stable action references and aggregate state synchronization; caller-owned error containment; explicit lifecycle disposal via manual `.dispose()` and automatic framework lifecycle integration (Angular accepts optional `{ destroyRef?: DestroyRef }` directly from `@angular/core`, Vue automatically registers `onScopeDispose` when called inside an active `effectScope` or accepts `options.onDispose`); preservation of canonical Form node lifecycles without auto-disposal on framework component teardown; clean package subpath separation keeping root `@vii-labs/form` framework-agnostic; and verified packed compatibility with Angular 17.3.12 through 22.1.4 and Vue 3.3.13 through 3.5.41.
Slice P1k implements the real-browser acceptance harness and accessibility regression gate for `@vii-labs/form` using Playwright in headless Chromium (`packages/form/test/browser/playwright.config.ts`, `pnpm nx test-browser form`). Features include: real DOM event semantics across input (text, checkbox, radio, file), textarea, and select-one; intermediate parsed number raw input retention (`"-"`, `"05"`) without presentation snap-back; simulated IME composition verification across Japanese, Korean, Chinese, and parser-backed sequences (proving zero intermediate validation runs and commits during active composition, and exactly one commit and validation run upon completion); focus and blur preservation with single blur validation invocation and activeElement retention during issue projection; WCAG 2.2 AA accessibility verification via `@axe-core/playwright` (0 automated violations), non-destructive `aria-invalid` lifecycle restoration, and additive `aria-describedby` linking; hostile payload plain text sink verification (0 DOM nodes created, 0 script execution); real-browser submit lifecycle (native submit button, Enter key, and unexpected exception containment with 0 unhandled Promise rejections); async validation race supersession and cancellation; route teardown unbinding without detached DOM mutation while canonical Form instances survive; React 19 real-browser lifecycle resilience across mount, edit, async validation unmount, and remount; and full verification of 15 cataloged historical regressions (`test/browser/HISTORICAL_REGRESSIONS.md`). Multi-select (`select-multiple`), public focus-first-invalid APIs, and Angular/Vue browser apps remain excluded.
Slice P1l implements the performance, bundle size, and memory lifecycle gate for `@vii-labs/form` (`packages/form/performance-budgets.json`, `docs/performance/FORM_P1L_BASELINE.md`, `pnpm nx performance form`). Features include: reproducible automated benchmarking across construction, mutation, validation, dynamic FieldArray collections, Model A submission, and framework adapter bridges; leaf-only mutation $O(1)$ scaling (0.48 µs at 1000 fields) with zero sibling notifications; aggregate value mutation $O(N)$ scaling (394 µs at 1000 fields); FieldArray mutations maintaining stable logical identity (<54 µs at 1000 items); memory lifecycle enforcement verifying 0 retained subscriptions, scopes, or timers across 500 create/dispose cycles, automatic debounce timer cancellation on disposal, and async validation supersession (200 mutations aborting 199 controllers with 1 commit and 0 unhandled rejections); bundle size budgets enforcing tree-shaking (createField standalone at 14.3 kB minified / 3.8 kB gzip, root form at 49.9 kB minified / 11.2 kB gzip, React adapter at 5.1 kB minified / 1.1 kB gzip, Vanilla at 8.5 kB minified / 2.3 kB gzip, Angular at 6.1 kB minified / 1.3 kB gzip, Vue at 5.7 kB minified / 1.2 kB gzip, packed tarball at 90.5 kB compressed / 1.06 MB unpacked); strict framework and schema isolation with 0 runtime bytes for `@standard-schema/spec`; and TypeScript compiler performance tracking (0.26s check time, 0 recursion errors).
Slice P1m completes the Vii Form Production Phase 1 graduation gate, graduating `@vii-labs/form` from Experimental to Preview Candidate (`vii.stability: "preview"` in `packages/form/package.json`). Features include: full public API and type declaration inventory audit across root and all framework adapters (18 runtime exports, 74 public named types); machine-readable API surface snapshot (`packages/form/api-surface.json`) enforced by package boundary test; package exports map disallowing unsupported deep internal imports (e.g. `@vii-labs/form/dist/core/field.js`); clean packed consumer matrix validation (root, vanilla, React 18 & 19, Angular 17 & 22, Vue 3.3 & 3.5); definitive 21-section consumer documentation (`packages/form/README.md`) backed by runnable test fixtures (`packages/form/test/unit/readme-examples.test.ts`); candidate Changeset (`.changeset/preview-form-candidate.md`); and release gate documentation (`docs/release/FORM_RELEASE_GATE.md`). In accordance with repository governance, the package remains private (`"private": true` in `package.json`), package publication is strictly deferred pending explicit maintainer release approval, zero changes were made to `@vii-labs/core`, zero new form features were added, and Phase 2 is not started. Vii Form Production Phase 1 is fully complete.
Slice P2a initiates Vii Form Phase 2 Planning. It establishes the `docs/architecture/FORM_PHASE2_ARCHITECTURE.md` architecture, gap inventory, competitor analysis (React Hook Form, TanStack Form, Angular Forms, VeeValidate), and the execution roadmap. It explicitly maps ownership, documents ADRs (e.g., dynamic registration, focus management, external state sync), and defines compatibility, security, and performance continuities. No Phase 2 runtime features are implemented. The package remains a Preview Candidate and publication is strictly deferred.
Slice P2b implements dynamic tree registration semantics for `@vii-labs/form`, introducing structural dynamic membership management to `Form` and `FieldGroup` via `register` and `unregister` without mutating component mounting lifecycle. Features include reactive `fieldKeysState` for derived statuses, submission race protections against concurrent mutations, a 1000-cycle structural stress matrix proving 0 residual leaks, and preservation of all P1l performance and bundle budgets.
Slice P2c implements cross-field validation and explicit dependency tracking for `@vii-labs/form`. Features include: explicit field dependency declarations (`dependencies: [fieldA]` or `(self) => [fieldA]`) for parserless and parsed fields; `ctx.get(depField)` context getter returning immutable snapshots captured at wave start; automatic topological validation waves using Kahn's algorithm ensuring downstream dependents revalidate dependencies-first with 0 duplicate executions; direct and transitive cycle detection throwing descriptive value-free errors; foreign form tree rejection and disposed node rejection; async validation race cancellation with AbortSignal across rapid mutations; seamless integration with dynamic tree registration (P2b) unregistering/disposing nodes without resurrection; compile-time type inference for `FieldDependenciesDeclaration` and `ctx.get()`; and full preservation of all P1l bundle, tree-shaking, and performance budgets.
Slice P2d implements DOM focus and accessibility orchestration for `@vii-labs/form` vanilla adapter. In accordance with ADR P2-3, focus management is owned strictly by the presentation adapter without touching or polluting `@vii-labs/core` or Form Core headless domain models. Features include: opt-in `focusInvalidOnSubmit` on `bindForm()` with custom fallback, first-invalid target resolver, and scroll behavior; programmatic `focusInvalid()` and `focusFirstInvalid()` methods on `VanillaFormBinding` and standalone functions; DOM document order resolution using `compareDocumentPosition` rather than object key order; radio group checked-first resolution; inert, hidden, and `aria-hidden="true"` ancestor skipping; custom control and container focus via `tabindex="-1"`; smooth scroll coordination (`scroll: true | ScrollIntoViewOptions`) and scroll-only mode (`focus: false`); lifecycle-safe registry association with automatic cleanup on binding disposal; full pass across all 41 hard performance, bundle, and memory gates with zero budget expansion; complete Playwright real-browser test suite with Axe accessibility audit yielding 0 WCAG violations; and clean consumer verification across all 8 clean workspace targets.
Slice P2e implements idiomatic React and Vue integrations for `@vii-labs/form` (`@vii-labs/form/react` and `@vii-labs/form/vue`). In accordance with ADR P2-1, one canonical state tree is preserved with zero duplicated form state, zero hidden external stores, and zero automatic structural unregistration on UI unmount. Under Preferred Option A, React adapter maintains pure component-level controller binding without coupling to Vanilla DOM focus orchestration or exposing fake no-op refs; consumers attach standard React refs directly to UI elements or native inputs. Features include: React `FormProvider` and `useFormContext` (reference-only transport avoiding provider re-renders); React `useController` hook and declarative `Controller` component with granular `field` (`value`, `onChange`, `onBlur`) and `fieldState` (`invalid`, `isTouched`, `isDirty`, `isValid`, `isPending`, `error`, `issues`); Vue `useViiField` composable with fine-grained reactive handles, two-way writable `model` computed property for `v-model`, and `bind()` helper; Vue `useViiForm` and `useViiFieldArray` composables; Vue `provideForm` and `useFormContext` dependency injection; Vue `vViiField` custom directive (`ViiFieldDirective`) providing overloaded template typing and runtime discriminated type guards (`isBooleanField`, `isStringField`) with zero unsafe setter casts and fail-closed safety for mismatched controls; granular reactivity verified via 100-field isolation tests; resource safety proven via 1,000-cycle lifecycle stress tests with 0 retained subscriptions; all 41/41 hard performance, bundle, and memory gates passing with zero budget weakening; and all 8 packed consumers (Root, Vanilla, React 18, React 19, Angular 17, Angular 22, Vue 3.3, Vue 3.5) passing cleanly.
Slice P2f implements Angular ecosystem integrations for `@vii-labs/form` (`@vii-labs/form/angular`). Signal Forms remain formally deferred (Option A). Features include: `ViiFieldDirective` (`[viiField]`) authored with public `@Directive` / `@Input` APIs and partial `ngc` compilation (host `ElementRef` injected by Angular; no private Ivy authoring in source); real TestBed template acceptance for input, textarea, and checkbox; `ViiControlValueAccessor` implementing the Angular Forms `ControlValueAccessor` protocol for use from a consumer host registered through `NG_VALUE_ACCESSOR`, with FormControl anti-loop tests; scoped DI via `VII_FORM_TOKEN`, `provideViiForm`, and `injectViiForm`; presentation-only teardown invariant; 100-field isolation and 1,000-cycle stress verification; 41/41 hard performance gates; packed consumers for Angular 17.3.12 and modern Angular compile and exercise real `[viiField]` templates from the published tarball; packed Angular 17 signal-only consumer validates `@angular/core` without `@angular/forms`.
Slice P2g expands Form browser infrastructure from Chromium-only Playwright to a mandatory three-engine matrix: Chromium, Firefox, and WebKit. The existing Form browser acceptance inventory (46 tests covering DOM binding, form interactions, P2d focus/a11y orchestration, Axe, dynamic unregister, lifecycle, IME, React 19, and historical regressions) runs identically on each Playwright-managed engine. CI installs Chromium, Firefox, and WebKit through one `playwright install --with-deps` step; a failure in any engine fails Validate. Public API, Form Core, and P1l performance budgets are unchanged.
Slice P2h is the Phase 2 graduation review. It audits the public API, package boundaries, docs, performance, packed consumers, cross-browser matrix, security/privacy/accessibility, and Phase 2 slice completion without adding features, publishing, or creating P2i. Current public API (from `packages/form/api-surface.json`): 35 runtime exports and 93 public named types across 5 preview entrypoints (root 7/50, React 7/12, Vanilla 4/9, Angular 8/9, Vue 9/13). Cross-browser matrix: Chromium 46/46, Firefox 46/46, WebKit 46/46 (138/138), Axe 0 violations on all engines. Packed consumers 8/8 (Root/Core-only, Vanilla, React 18.3.1, React 19.2.8, Angular 17.3.12, Angular 22.1.4, Vue 3.3.13, Vue 3.5.41) plus Angular 17 signal-only / no-`@angular/forms`. Performance 41/41 HARD; budgets unchanged. Memory: 0 retained subscriptions/scopes/timers, 0 stale commits, 0 unhandled rejections. Production source, `api-surface.json` semantics, and performance budgets have zero P2h delta. Phase 2 product development is complete. Preview readiness is confirmed. `@vii-labs/form` remains Preview (`vii.stability: "preview"`), private (`"private": true`), version `0.1.0-experimental.1`, and unpublished. Publication remains a separate explicit maintainer gate. No P2i. Phase 3 Form work is not started.
`@vii-labs/form` declares `@vii-labs/core` as a required runtime peer (`>=0.1.0-experimental.2`),
`@standard-schema/spec` as a type-consumed dependency, and optional peers: `@angular/core` (Angular signal adapter and `[viiField]` directive), `@angular/forms` (Angular Forms CVA bridge types and host integration), `react`, and `vue`. It has zero runtime dependency on `research/form/` and makes zero modifications to `@vii-labs/core`.
...
Production Form Phase 1 (P1a–P1m) and Phase 2 (P2a–P2h) product development are complete. Preview readiness is confirmed. `@vii-labs/form` remains Preview, private, and unpublished. Publication remains a separate explicit maintainer gate. No P2i. Phase 3 Form work is not started. Architecture and slice history live in `docs/architecture/FORM_ARCHITECTURE.md` and `docs/architecture/FORM_PHASE2_ARCHITECTURE.md`.
