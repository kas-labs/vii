...
Slice P2e implements idiomatic React and Vue integrations for `@vii-labs/form` (`@vii-labs/form/react` and `@vii-labs/form/vue`). In accordance with ADR P2-1, one canonical state tree is preserved with zero duplicated form state, zero hidden external stores, and zero automatic structural unregistration on UI unmount. Under Option B, React adapter maintains pure component-level controller binding and consumer callback ref without coupling to Vanilla DOM focus orchestration. Features include: React `FormProvider` and `useFormContext` (reference-only transport avoiding provider re-renders); React `useController` hook and declarative `Controller` component with granular `field` (`value`, `onChange`, `onBlur`, `ref`) and `fieldState` (`invalid`, `isTouched`, `isDirty`, `isValid`, `isPending`, `error`, `issues`); Vue `useViiField` composable with fine-grained reactive handles, two-way writable `model` computed property for `v-model`, and `bind()` helper; Vue `useViiForm` and `useViiFieldArray` composables; Vue `provideForm` and `useFormContext` dependency injection; Vue `vViiField` custom directive enabling declarative two-way DOM binding for supported controls (`<input>`, `<textarea>`, `<input type="checkbox">` with string/boolean raw representation) with clean listener and subscription teardown; granular reactivity verified via 100-field isolation tests; resource safety proven via 1,000-cycle lifecycle stress tests with 0 retained subscriptions; all 41/41 hard performance, bundle, and memory gates passing with zero budget weakening; and all 8 packed consumers (Root, Vanilla, React 18, React 19, Angular 17, Angular 22, Vue 3.3, Vue 3.5) passing cleanly.
